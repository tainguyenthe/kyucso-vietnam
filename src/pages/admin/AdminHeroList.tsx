...
export function AdminHeroList() {
  ...
  const fetchHeroes = () => {
    setLoading(true)
    getAllHeroes(page)
      .then(({ heroes: h, total: t }) => { setHeroes(h); setTotal(t) })
      .catch(() => toast.error('Lỗi tải dữ liệu'))
      .finally(() => setLoading(false))
  }

  useEffect(fetchHeroes, [page])
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h2 className="font-serif text-2xl font-bold text-maroon-800">Anh hùng - Liệt sĩ</h2>
        <Link
          to={ROUTES.ADMIN_HERO_CREATE}
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-maroon-50 border-b border-maroon-200">
                ...
                <th className="text-left px-4 py-3 font-medium text-maroon-700">Cấp bậc</th>
            ...
            <tbody>
              {loading ? (
                ...
                heroes.map((hero) => (
                  <tr key={hero.id} className="border-b border-maroon-100 hover:bg-maroon-50">
                    <td className="px-4 py-3 font-medium text-maroon-800">{hero.full_name}</td>
                    <td className="px-4 py-3 text-maroon-600">{hero.rank ?? '-'}</td>
                    <td className="px-4 py-3 text-maroon-600">{hero.era}</td>
                    <td className="px-4 py-3">
                      <span className={`text-xs px-2 py-0.5 rounded-full ${
                        hero.status === 'published' ? 'bg-green-100 text-green-700' : 'bg-yellow-100 text-yellow-700'
                      }`}>
                        {hero.status === 'published' ? 'Công khai' : 'Bản nháp'}
                      </span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        <Link
                          to={`/admin/anh-hung/${hero.id}/chinh-sua`}
                        ...
                        <button
                          onClick={() => handleDelete(hero.id, hero.full_name)}
